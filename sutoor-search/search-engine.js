...
(function () {
  ...
  var el = {}, state = {
    ...
    hits: [], total: 0, rendered: 0, ready: false, workerReady: false, expanded: false
  };
  ...
  function boot(config) {
    ...
    idle(function () {
      config = config || {};
      loadBuiltIndex(config).then(function (idx) {
        if (idx) return idx;
        return new Sutoor.Indexer(config).run();
      }).then(function (idx) {
        index = idx;
        startWorker(idx);
      }).catch(function (e) {
  ...
  /** يفضّل فهرس Netlify Build الجاهز، ويعود للمسار الحالي إن لم يتوفر. */
  function loadBuiltIndex(config) {
    if (config.builtIndex === false || typeof fetch !== 'function') return Promise.resolve(null);
    var url = config.builtIndexUrl || window.SUTOOR_BUILT_INDEX_URL || '/sutoor-search/search-index.json';
    setStat('يحمّل فهرس الأرشيف الجاهز…');
    return fetch(url, { credentials: 'same-origin', cache: 'no-cache' }).then(function (response) {
      if (!response.ok) return null;
      return response.json();
    }).then(function (idx) {
      if (!idx || !Array.isArray(idx.docs) || !Array.isArray(idx.terms) || !idx.postings) return null;
      return idx;
  ...
  function startWorker(idx) {
    ...
    try {
      ...
      worker.onerror = function () { worker = null; state.workerReady = false; useFallback(idx); };
  ...
  var api = {
    ...
    stats: function () { return { docs: index ? index.docs.length : 0, ready: state.ready, worker: state.workerReady }; }
  };
